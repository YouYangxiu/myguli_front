<template>
  <div id="aCoursesList" class="bg-fa of">
                  ...
                  <section class="mt20">
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          <!-- 文件目录 -->
                          <li class="lh-menu-stair" v-for="chapter in chaptersVideosList" :key="chapter.id">
    ...
    <!-- /课程详情 结束 -->
    <!--    <button type="button" @click="showInfo">点我展示信息！！！</button>-->
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getCourseInfo() {
      courseMethod.getCourseInfo(this.$route.params.id).then(response => {
        // console.log(response);
        this.courseWebVo = response.data.data.courseWebVo
        this.chaptersVideosList = response.data.data.chaptersVideosList
    ...
    //生成订单
    createOrders() {
      console.log('课程id是。..', this.courseId)
      orderMethod.crateOrder(this.courseId).then(response => {
        //获取返回的订单号,跳转到订单显示页面
