...
import registerMethod from '@/api/register'
...
export default {
  ...
  methods: {
    submitRegister() {
      registerMethod.registerMember(this.params).then(response => {
    ...
    getCodeFun() {
      registerMethod.sendCodeByMobile(this.params.mobile).then(response => {
        this.timeDown();
        this.sending = false;
      });
    },
