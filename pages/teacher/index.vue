...
<script>
import teacherMethod from "@/api/teacher";


export default {
  //异步调用 调用一次
  //params：相当于之前this.$route.params.id 等价params.id
  asyncData({params, error}) {
    return teacherMethod.getTeacherList(1, 8).then(response => {
      return {
        data: response.data.data.pageInfo
      }
    })
  },
  methods: {
    gotoPage(page) {
      teacherMethod.getTeacherList(page, 8).then(response => {
        this.data = response.data.data.pageInfo;
      })
    }
  }
};
